...
class RiskEvent(Base):
    ...
    risk_type = Column(String(50), CheckConstraint("risk_type IN ('port_congestion', 'customs_delay', 'weather', 'quality_hold', 'carrier_delay', 'route_disruption')"), index=True)
...
class AgentDecision(Base):
    ...
    agent_name = Column(String(50), CheckConstraint("agent_name IN ('risk_detection', 'mitigation_planner', 'execution', 'communication', 'simulation')"))
    decision_type = Column(String(50), CheckConstraint("decision_type IN ('risk_assessment', 'mitigation_selection', 'action_execution', 'escalation')"))
...
class MitigationAction(Base):
    ...
    event_id = Column(String(50), ForeignKey('risk_events.event_id'), index=True)
    shipment_id = Column(String(50), ForeignKey('shipments.shipment_id'), index=True)
    action_type = Column(String(50), CheckConstraint("action_type IN ('reroute', 'mode_switch', 'carrier_change', 'expedite', 'split_shipment', 'notify_customer')"))
    description = Column(Text)
    estimated_cost_usd = Column(Numeric(15, 2))
