...
def process_risk_event(event_id):
    """
    Process a single risk event triggered by the database listener.
    This function is called when a risk event with status='detected' is inserted or updated.
    
    Args:
        event_id: The ID of the risk event to process
        
    Returns:
        dict: Processing result with status and details
    """
    logger.info(f"Processing risk event: {event_id}")
    ...
    try:
        with get_db_context() as db:
            # Fetch the risk event
            event = db.query(RiskEvent).filter(RiskEvent.event_id == event_id).first()
            
            if not event:
                logger.error(f"Risk event {event_id} not found.")
                return {"error": "Risk event not found", "event_id": event_id}
            ...
            if not shipment:
                logger.warning(f"No shipment associated with event {event_id}")
                return {"error": "No shipment associated with event", "event_id": event_id}
        ...
        logger.error(f"Error processing risk event {event_id}: {e}")
